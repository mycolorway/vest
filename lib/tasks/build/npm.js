...
const {resolveDependencies, resolveMiniprogramDistPath} = require('../utils')
const {getMiniprogramDistPath} = require('../../commands/utils')

module.exports = function(gulp) {
  const modulesPath = path.resolve(this.config.cwd, 'node_modules')
    ...
    if (mpDistPath && fs.existsSync(mpDistPath)) {
      gulp.task(taskName, () => {
        ...
        return gulp.src(path.join(mpDistPath, '**/*'))
          .pipe(gulpIf(this.config.lark, resolveMiniprogramDistPath(modulesPath)))

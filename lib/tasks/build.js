...
class BuildTasksRegistry extends BaseRegistry {
  ...
  init(gulp) {
    ...
    gulp.task(`${this.config.name}:watch`,(done) => {
      ...
      log('start watching file changes...')
      done()
    })

    const buildTasks = [
      (done) => {
        log(`start building project: ${this.config.projectName}(${this.config.env})`)
        done()
      },
      `${this.config.name}:clean`,
      gulp.parallel(this.taskNames.map(name => `${this.config.name}:${name}`)),
      (done) => {
        log(`finish building project: ${this.config.projectName}(${this.config.env})`)
        done()
      }
    ]
